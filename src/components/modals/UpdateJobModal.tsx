...
import { updateJob, Job } from '../../features/jobs/jobsSlice';
...
export const UpdateJobModal: React.FC<UpdateJobModalProps> = ({ job, isOpen, onClose }) => {
  ...
  useEffect(() => {
    if (job) {
      setFormData({
        ...
        experience_required: job.experience?.toString() || '',
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await dispatch(updateJob({ jobId: job.id, jobData }));
      onClose();
    } catch (error) {
